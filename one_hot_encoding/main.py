...
def train(args, emb_model, model, train_loader, val_seen_loader, val_unseen_loader, criterion, optimizer, scheduler, device):
    ...
    for epoch in range(start_epoch, args.n_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            loss = criterion(preds.to(device), labels.to(device))
        ...
        # np.savetxt(os.path.join(args.pred_dir, f"val_seen_pred_epoch{epoch+1}_score{seen_score:.4f}.csv"),  seen_save_list, fmt='%s', delimiter=',')
        unseen_score, unseen_save_list = valid(args, model, val_unseen_loader, criterion, "Valid_unseen", device)
        # np.savetxt(os.path.join(args.pred_dir, f"val_unseen_pred_epoch{epoch+1}_score{unseen_score:.4f}.csv"),  unseen_save_list, fmt='%s', delimiter=',')
        ...
        if avg_score >= max_avg_score:
            ...
            torch.save(checkpoints, os.path.join(args.dataset_type, f"model_best_epoch{epoch+1}_seen{max_seen_score:.4f}_unseen{max_unseen_score:.4f}.pth"))
            # if not args.fix_encoder:
            #     torch.save(emb_model.state_dict(), os.path.join(args.model_dir, args.dataset_type, f"emb_model_best_epoch{epoch+1}_seen{max_seen_score:.4f}_unseen{max_unseen_score:.4f}.pth"))
        ...
        torch.save(checkpoints, os.path.join( args.dataset_type, f"model_last.pth"))
        # if not args.fix_encoder:
...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(args.dataset_type), exist_ok=True)
    # os.makedirs(os.path.join(args.pred_dir, args.dataset_type), exist_ok=True)
